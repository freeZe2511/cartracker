...
class PositionController {
  Future<Response> createPos(Request request) async {
    ...
    await CoordinateDao.create(Coordinate(id: userid, lat: lat, lng: lng, inZone: inZone));
  ...
  Future<bool> isInZone(String userid,double lat, double lng) async {
    ...
    if(z != null){
      ...
      if(z.radius > 0){
        return checkCircleZone(z.pos[0]["lat"], z.pos[0]["lng"], lat, lng, z.radius);
      }
    }
    return true;
  }

  bool checkCircleZone(double zoneLat,double zoneLng,double userLat,double userLng, num radius) { // radius in m
    const radiusEarth = 6371; // in km
    double distanceLat = radians(userLat - zoneLat);
    double distanceLon = radians(userLng - zoneLng);
    num a = sin(distanceLat/2) * sin(distanceLat/2) + cos(radians(zoneLat)) * cos(radians(userLat)) * sin(distanceLon/2) * sin(distanceLon/2);
